...
sample_analysis <- function(unique_site){
  ...
  for (unique_metal in metals_analyzed){
    ...
    sample_data <- filter(ICPMS, metal == unique_metal, site == unique_site, sample.key==ID)
   ...
   if (nrow(sample_data)==1){
    ...
    #Error Propagation
      ...
      e_yb <- sqrt((sample_data$RSD.corrected)^2 + (intercept.error)^2) 
      #error in y-b from the calibration
      yb <- sample_data$CPS.corrected-b
      e_x <- x*sqrt((e_yb/yb)^2+(slope.error/m)^2)
    ...
    if (unique_site=="MB"){
      x <- mean(x)
      e_x <- propmean(e_x, x)
      RSD.corrected <- propmean(sample_data$RSD.corrected, sample_data$CPS.corrected)
      CPS.corrected <- mean(sample_data$CPS.corrected)
      }}}
