...
sample_analysis <- function(unique_site){
  ...
  for (unique_metal in metals_analyzed){
    ...
    for(ID in sample$sample_key){ 
      ...
      data <- rbind(data, data_frame(sample_key = ID, x, e_x)) #storing for MB purposes
      if (unique_site != "MB"){
...
keys <- unique(uncor_sample$sample_key) #sample_keys that correspond to actual samples! 
#(i.e. not QC, MB)
